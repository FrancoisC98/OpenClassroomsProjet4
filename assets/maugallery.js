...
prevImage() {
  ...
  // Remplir la collection d'images selon le tag
  if (activeTag === "all") {
  ...
  // Passer à l'image précédente, en mode circulaire
  index = (index - 1 + imagesCollection.length) % imagesCollection.length;

  // Mettre à jour la lightbox avec la nouvelle image
...
nextImage() {
  ...
  // Construire la collection d'images selon le tag actif
  if (activeTag === "all") {
    $(".item-column").each(function () {
      if ($(this).children("img").length) {
        imagesCollection.push($(this).children("img"));
  ...
  // Passer à l'image suivante en mode circulaire
